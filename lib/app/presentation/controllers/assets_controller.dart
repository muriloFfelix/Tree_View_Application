...
class AssetsController extends GetxController {
  ...
  final List<TreeItemTileData> itemsFlatTree = [];
  ...
  List<TreeItemTileData> getTreeList(
      {String? parentId, List<String>? currentPath}) {
    final List<String> idPath = currentPath ?? [];

    for (var item in getItemsGroup(parentId.toString())) {
      if (filterIds.isEmpty || filterIds.contains(item.id)) {
        final int depth = _getDepth(idPath, item);

        if (depth == 0) {
          idPath.clear();
        } else {
          idPath.removeRange(depth, idPath.length);
        }
        idPath.add(item.id);

        itemsFlatTree.add(TreeItemTileData(entity: item, depth: depth));

        if (!closedIds.contains(item.id)) {
          getTreeList(parentId: item.id, currentPath: idPath);
        }
      }
    }
  ...
  int _getDepth(List<String> idPath, BasicEntity item) {
    final String? id = (item is AssetEntity && item.locationId != null)
        ? item.locationId
        : item.parentId;

    if (id != null && idPath.contains(id)) {
      final index = idPath.indexOf(id);
      return index + 1;
    } else {
      return 0;
    }
